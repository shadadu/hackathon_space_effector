...
def rollout_micro_g_dgm_policy(
    ...
    for _ in range(attempts):
        ...
        for t in ts:
            t_s = float(np.asarray(t).reshape(-1)[0])
            ...
            if np.linalg.norm(r) <= cfg.grasp_pos_tol and np.linalg.norm(v_rel) <= cfg.grasp_vel_tol:
                if cfg.require_final_reachable and not is_target_reachable(
                        object_base_dist,
                        cfg.reach_min,
                        cfg.reach_max,
                        cfg.reach_margin,
                    ...
                    rospy.logwarn(
                        "Rejecting grasp at rollout step %d: object/base distance %.3f outside reach shell [%.3f, %.3f]",
                        k,
                        object_base_dist,
                        cfg.reach_min,
                        cfg.reach_max,
            ...
            rospy.loginfo("p_o  = %s, b = %s, ee_pos = %s, ee_local_pos = %s", p_o, b, p_ee, p_ee_local)
            rospy.loginfo(
            ...
            k += 1
            last_valid_len = max(last_valid_len, k)
